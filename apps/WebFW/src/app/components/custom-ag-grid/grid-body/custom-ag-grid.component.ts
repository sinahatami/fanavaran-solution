...
export class CustomAgGridComponent implements OnInit {
  ...
  setGridOption() {
    if (!this.isModal) {
      ...
      this.gridOption.frameworkComponents = frameworkComponents
    }
    else this.gridOption.frameworkComponents = frameworkComponentsSimple
  ...
  gridOption = <GridOptions>{
    ...
    //getContextMenuItems: this.activeContextList
  }
  ...
  setGrid() {
    ...
    else this.gridOption.columnDefs.map((a: any) => { a.isModal = this.isModal, a.width != 0 ? a.maxWidth = a.width : a.width = null, a.type == 'int' ? a.valueFormatter = function (params) { if (params.value) return Math.floor(params.value).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') } : null, a.headerTooltip = a.headerName, a.floatingFilterComponent = 'floatfilter', a.filter = 'agTextColumnFilter', a.type == 'bool' ? a.cellRendererFramework = CheckboxRendererComponent : a.cellRendererFramework = CellRendererComponent, a.cellRendererParams = { type: a.type } })
  ...
  setContextMenu() {
    let context: any
    let activeContextList: any[]
    const contextNameList: any[] = this.customGridOption.contextMenu
    contextNameList.forEach(e => {
      context = this.bigListContext.filter(a => a['name'] == e)[0]
      activeContextList.push({ name: context.name, function: context.function })
    })
    this.activeContextList(activeContextList)
  }
